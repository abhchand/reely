...
RSpec.describe 'users/registrations/new.html.erb', type: :view do
  ...
  it 'renders the auth form' do
    ...
    expect(first_name['placeholder']).to eq(
      User.human_attribute_name(:first_name)
    )

    expect(page.find("label[for='user_first_name']")).to have_content(
      User.human_attribute_name(:first_name)
    )
    ...
    expect(last_name['placeholder']).to eq(
      User.human_attribute_name(:last_name)
    )

    expect(page.find("label[for='user_last_name']")).to have_content(
      User.human_attribute_name(:last_name)
    )
