...
RSpec.describe ContentImportJob, type: :worker do
  ...
  it 'creates a Photo with source file attachment' do
    ...
    expect { ContentImportJob.new.perform(owner.id, filepath) }.to change {
      Photo.count
    }.by(1)
  ...
  describe 'file and directory cleanup' do
    it 'removes the file and directory' do
      file_and_dir_removed =
        proc { !filepath.exist? && !filepath.dirname.exist? }

      expect { ContentImportJob.new.perform(owner.id, filepath) }.to change {
        file_and_dir_removed.call
      }.from(false).to(true)
    end
    ...
    context 'other files still exist in the directory' do
      ...
      it 'removes the file but not the directory' do
        ...
        expect { ContentImportJob.new.perform(owner.id, filepath) }.to_not(
          change { filepath.dirname.exist? }
        )
