...
RSpec.describe Collections::DownloadsController, type: :controller do
  ...
  describe 'GET #show' do
    let(:params) { { collection_id: collection.share_id, id: encrypted_id } }
    ...
    it 'returns the download' do
      ...
      expect(controller).to(
        receive(:send_file).with(filepath, type: mime_type).and_call_original
      )
  ...
  describe 'POST #create' do
    let(:params) { { format: 'json', collection_id: collection.share_id } }
    ...
    context 'user is not authenticated' do
      ...
      it 'still calls the controller action' do
        expect { post :create, params: params }.to(
          change { BundleFilesJob.jobs.size }.by(1)
        )
      end
    end
  ...
  describe 'GET #status' do
    ...
    context 'encrypted id is invalid' do
      ...
      it 'returns the status as incomplete' do
        ...
        expect(response.code).to eq('200')
        expect(JSON.parse(response.body)).to eq(
          'download' => { 'isComplete' => false, 'url' => nil }
        )
      end
    end
