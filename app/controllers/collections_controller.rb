class CollectionsController < ApplicationController
  ...
  before_action :ensure_json_request,
                only: %i[update add_photos remove_photos destroy]
  ...
  def create
    @collection =
      Collection.new(
        owner: current_user, name: collections_create_params['name']
      )
  ...
  def update
    ...
    respond_to { |format| format.json { render json: {}, status: status } }
  end
  ...
  def add_photos
    service =
      AddPhotosToCollection.call(
        collection: @collection, params: collections_add_or_remove_photos_params
      )
  ...
  def remove_photos
    service =
      RemovePhotosFromCollection.call(
        collection: @collection, params: collections_add_or_remove_photos_params
      )
    ...
    respond_to { |format| format.json { render json: meta, status: status } }
  end
  ...
  def collections_add_or_remove_photos_params
    params.require(:collection).permit(photo_ids: [])
  end
end
